// lib/mcp/tools/add-boolean-field.ts
import { z } from "zod";
...
const inputSchema = {
	...
	name: z
		.string()
		.describe("Field name (e.g., 'isActive', 'isVerified', 'hasAccess')"),
	required: z
		.boolean()
		.optional()
		.describe("Field is required in every document"),
	default: z.boolean().optional().describe("Default value (true/false)"),
	description: z
		.string()
		.optional()
...
const handler: ToolCallback<ZodRawShapeCompat> = async (args, extra) => {
	...
	try {
		...
		const fieldDef = buildFieldDef(name, "boolean", { description });
		...
		const result = {
			...
			field: name,
			added: true,
			message: `Field '${name}' (boolean) added successfully to '${collection}'`,
		};
...
// EXPORT
// ============================================================
export const addBooleanFieldTool = {
	name: "add_boolean_field",
	config: {
		title: "Add Boolean Field",
		description:
			"[Schema] Add a boolean field to a collection with required flag, default value, and description.",
		inputSchema,
		outputSchema,
		annotations: {
			title: "Add Boolean Field",

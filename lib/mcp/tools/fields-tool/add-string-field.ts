// lib/mcp/tools/add-string-field.ts
import { z } from "zod";
...
const inputSchema = {
	...
	name: z
		.string()
		.describe("Field name (e.g., 'email', 'username', 'title')"),
	type: z
		.enum(["string", "email", "url", "phone", "zipCode"])
		.describe(
			"String type. email/url/phone/zipCode auto-apply regex pattern",
		),
	required: z
		.boolean()
		.optional()
	...
	pattern: z
		...
		.optional()
		.describe(
			"Custom regex pattern (leave empty for email/url/phone/zipCode auto-pattern)",
		),
	...
	try {
		...
		if (
			...
			const errorResult = {
				...
				added: false,
				message:
					"INVALID_FIELD_VALIDATION: minLength cannot be greater than maxLength",
			};
			return {
				content: [
					{
		...
		const fieldDef = buildFieldDef(name, type, {
			minLength,
			maxLength,
			pattern,
			description,
		...
		const result = {
			...
			field: name,
			added: true,
			message: `Field '${name}' (${type}) added successfully to '${collection}'`,
		};
...
// EXPORT
// ============================================================
export const addStringFieldTool = {
	name: "add_string_field",
	config: {
		title: "Add String Field",
		description:
			"[Schema] Add a string/email/url/phone/zipCode field to a collection with validation (minLength, maxLength, pattern), required flag, unique index, and default value.",
		inputSchema,
		outputSchema,
		annotations: {
			title: "Add String Field",
